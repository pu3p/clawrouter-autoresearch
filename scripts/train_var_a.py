"""Variation A: Freeze backbone for 1 epoch, then unfreeze."""
...
CKPT_DIR = os.path.join(
    RESULTS_DIR, "classifier_checkpoints/var_a"
)
NUM_DOMAINS = 57
NUM_COMPLEXITY = 4
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        optimizer = torch.optim.AdamW(
            opt_params, lr=LR, weight_decay=WEIGHT_DECAY
        )
        steps_this_epoch = len(train_loader) // GRAD_ACCUM
        scheduler = get_linear_schedule_with_warmup(
            optimizer,
            num_warmup_steps=int(
                steps_this_epoch * WARMUP_RATIO
            ),
            num_training_steps=steps_this_epoch,
        )
    ...
    with open(
        os.path.join(RESULTS_DIR, "var_a_metrics.json"), "w"
    ) as f:
        json.dump(metrics_log, f, indent=2)
    print("Variation A complete.")
